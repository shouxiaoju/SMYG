        ...
        <li v-for="(item, index) in list" :key="index">
          ...
          <div class="xiangq">
            ...
            <img
              src="../../../../assets/下载.png"
              alt=""
              :class="[a && rotate == index ? 'go' : 'aa']"
              @click.self="start(index)"
            />
          </div>
          <div class="xiangq1" v-if="rotate == index && a">
            <span>
              <p v-for="(item, index) in item.arr_price.l" :key="index">
                {{ item }}:
              </p>
            </span>
            <span>
              <p v-for="(item, index) in item.arr_price.r" :key="index">
                {{ item }}
              </p>
            </span>
          </div>
  ...
  methods: {
    jcar (index) {
      if (localStorage.getItem('token')) {
        var uname = JSON.parse(localStorage.getItem('token')).uid
        var num = JSON.parse(localStorage.getItem('token')).token
        // console.log(uname)
        this.$http
          .get(
            `/api/index.php/index/order/addCart?token=${num}&uid=${uname}&temporary_token=&jingying_type=1&type=1&item_id=${index}&item_num=1`
          )
          .then(res => {
            if (res) {
              alert('已加入购物车')
      ...
      if (het - top - clhet < 300) {
        ...
        if (this.c === 0) {
          if (this.status === 200) {
            ...
            this.$http
              .get(
                `/api/index.php/index/item/itemlist?page=${this.page1}&str_keyword=${this.url}&fzFiltrate=&brandFiltrate=&priceSort=&encap=&jingying_type=`
              )
              .then(res => {
          ...
          if (this.status === 200) {
            ...
            this.$http
              .get(
                `/api/index.php/index/item/itemlist?page=${this.page1}&str_keyword=${this.url}&fzFiltrate=&brandFiltrate=&priceSort=asc&encap=&jingying_type=`
              )
              .then(res => {
                if (res.data.status === 500) {
                  this.finished = true
          ...
          if (this.status === 200) {
            ...
            this.$http
              .get(
                `/api/index.php/index/item/itemlist?page=${this.page1}&str_keyword=${this.url}&fzFiltrate=&brandFiltrate=&priceSort=desc&encap=&jingying_type=`
              )
              .then(res => {
                if (res.data.status === 500) {
                  this.finished = true
    ...
    onSearch () {
      ...
      this.$http
        .get(
          `/api/index.php/index/item/itemlist?page=${this.page1}&str_keyword=${this.value}&fzFiltrate=&brandFiltrate=&priceSort=&encap=&jingying_type=`
        )
        .then(res => {
          this.status = res.data.status
          if (this.status === 500) {
    ...
    //点击动画
      ...
      //跳详情
      tiaozhuan(name){
        this.$router.push({path:'/detail',query:{search:name}})
